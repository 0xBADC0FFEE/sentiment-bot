...
import { alenkaAuth, toMessage, detectAuthorAlerts, detectHotAlerts } from "./index.js"
...
export async function runAuthors(opts: AlenkaOpts = {}): Promise<AuthorsResult> {
  const { store, api, cookie } = await resolve(opts)
